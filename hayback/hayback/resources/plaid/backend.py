...
DDB_TABLE_NAME = 'Bluechip' #os.environ.get('DDB_TABLE_NAME')
...
def get_transactions(access_token, start_date = '2017-09-29', end_date = '{:%Y-%m-%d}'.format(datetime.now())): 
    try:
      transactions_response = client.InvestmentTransactions.get(access_token, start_date, end_date)
...
def get_holdings(access_token): 
    ...
    return holdings_response['investment_transactions']
